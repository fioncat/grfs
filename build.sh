...
echo ""
echo "Building..."
CGO_ENABLED=0 GOOS=${GOOS} GOARCH=${GOARCH} go build -ldflags "-X main.Version=${BUILD_VERSION} -X main.BuildType=${BUILD_TYPE} -X main.BuildCommit=${GIT_COMMIT} -X main.BuildTime=$(date +%F-%Z/%T)" -o ${OUTPUT}
if [[ $? -ne 0 ]]; then
	echo "Build failed"
	exit 1
fi
